 ...
 * Each tool additionally applies its own ignore config (oxfmt reads .gitignore
 * and .oxfmtrc ignorePatterns; oxlint uses .oxlintrc ignorePatterns).
 */
...
// .oxlintrc); given explicit filenames those tools exit non-zero. Swallow it so
// an all-ignored change set still passes.
...
function run(job: Job): Promise<{ label: string; ok: boolean }> {
  return new Promise(resolve => {
    // shell:true so the pnpm launcher resolves on Windows too. Args are chunked
    // (see chunkByLength) to stay under the OS command-line limit.
